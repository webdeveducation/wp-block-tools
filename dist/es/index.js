...
function Query({ block, allBlocks }) {
    ...
    return (React.createElement(React.Fragment, null, (innerBlocks || []).map((topInnerBlock) => {
        ...
        if (topInnerBlock.name === 'wp-block-tools/loop') {
            ...
            return (React.createElement("nav", { key: topInnerBlock.id, className: `${getClasses(topInnerBlock)} wp-block-query-pagination`, style: getStyles(topInnerBlock) }, (_a = topInnerBlock.innerBlocks) === null || _a === void 0 ? void 0 : _a.map((innerBlock) => {
                var _a;
                ...
                switch (innerBlock.name) {
                    case 'core/query-pagination-previous': {
                        return (React.createElement("a", { key: innerBlock.id, href: `?query-${queryId}-page=${parseInt(currentPage) - 1}`, style: getLinkTextStyle(topInnerBlock.attributes), className: "wp-block-query-pagination-previous", onClick: (e) => {
                                e.preventDefault();
                                window.history.pushState({ path: e.target.href }, '', e.target.href);
                                // load here
                    ...
                    case 'core/query-pagination-next': {
                        return (React.createElement("a", { key: innerBlock.id, href: `?query-${queryId}-page=${parseInt(currentPage) + 1}`, className: "wp-block-query-pagination-next", style: getLinkTextStyle(topInnerBlock.attributes), onClick: (e) => {
                                e.preventDefault();
                                window.history.pushState({ path: e.target.href }, '', e.target.href);
                                // load here
                                setCurrentPage(`${parseInt(currentPage) + 1}`);
                            ...
                            "Next Page",
                            ' ',
                            paginationArrow === 'arrow' ? (React.createElement("span", { className: "wp-block-query-pagination-next-arrow is-arrow-arrow" }, "\u2192")) : paginationArrow === 'chevron' ? (React.createElement("span", { className: "wp-block-query-pagination-next-arrow is-arrow-chevron" }, "\u00BB")) : ('')));
                    }
                    case 'core/query-pagination-numbers': {
                        return (React.createElement("div", { key: innerBlock.id, className: "wp-block-query-pagination-numbers" }, Array.from({
                            ...
                            return (React.createElement(PaginationPageNumber, { key: i, pageNumber: i + 1, queryId: innerBlock.attributes.queryId, onClick: handlePageClick, style: getLinkTextStyle(topInnerBlock.attributes) }));
                        })));
                    }
                    default:
                        return null;
